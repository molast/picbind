...
| 职责 | 文件 |
|---|---|
| 前端统一入口、格式编码和回退 | `web/src/utils/compress-algorithms.ts` |
| 共享 WebP / AVIF 浏览器编码 SDK | `sdk/wasm/image-codecs/src/index.ts` |
| PCE 前端 Plan、编码器选择和 Butteraugli 调度 | `web/src/utils/compression-engine.ts` |
| Gain 环境变量和前端公式 | `web/src/utils/compression-gain.ts` |
| Worker 生命周期 | `web/src/utils/wasm-worker.ts`、`web/src/workers/compress.worker.ts` |
...
| PNG / imagequant / lodepng | `wasm/image_wasm/src/core/png.rs` |
...
3. Web 与 Room 的 WebP、AVIF 浏览器编码统一通过 `sdk/wasm/image-codecs` 提供的 `@picbind/image-codecs`。SDK 内部按格式拆分实现：WebP 适配层持有普通/SIMD WASM，统一执行 SIMD 能力检测、Emscripten 模块缓存、编码实例中止后的重建和 SIMD 失败后的普通 WASM 兜底；AVIF 适配层持有单线程/多线程 WASM 和线程 Worker 资源，并共享线程能力判断、模块缓存、大图分块参数、编码实例中止后的重建和多线程失败后的单线程兜底。Room 不再维护另一套 jsquash WebP/AVIF 初始化流程。保持原始尺寸时，两种格式都通过 `createImageBitmap` 解码为 RGBA，并在 `finally` 中关闭 `ImageBitmap`；传入目标尺寸时改用第 4 条的 WASM RGBA 缩放路径，不生成 Canvas 中间图片。WebP 的普通/SIMD WASM 通过 SDK 的 `locateFile` 映射；AVIF 不把 pthread Worker 手动输出为裸 `?url` 静态文件，而是保留 Emscripten 主模块、线程 Worker 和 Worker 内动态导入的原生模块关系，交由 Webpack/Vite 在同一打包上下文中生成可访问 URL，避免 Worker 找不到 `avif_enc_mt.js`。Web 主项目仍将显式带 `?url` 的 `.wasm` 和 `.mjs` 声明为静态资源。`@jsquash/avif` 的 pthread Worker 内部包含运行时动态导入，主模块与线程 Worker也会形成 Emscripten 固有的循环 chunk；Next 只按该 Worker 模块和实际包含 AVIF pthread 模块的 chunk 精确过滤这两类已知构建警告，不屏蔽其他 Critical dependency 或循环 chunk 警告。共享 Rust 产物分别由 `@picbind/image-wasm` 和 `@picbind/perceptual-wasm` 包提供，生成的 JS、类型声明和 WASM 二进制随 SDK 提交；Web、Room 与 image-codecs 由 `web/pnpm-workspace.yaml` 统一安装，各包仍只在自己的 `package.json` 中声明内部依赖。
...
- 色彩全尺寸处理与直接编码 Worker：`sdk/room/src/workers/room-color-adjustment.worker.ts`
