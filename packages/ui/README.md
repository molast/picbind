...
```tsx
import { ShareRoomPage } from "@picbind/ui";
...
The browser always connects to the online realtime Worker at `https://api.picbind.com`. Placeholder and thumbnail generation use the shared artifacts in `packages/wasm/image-wasm`; consumers may still set `wasmBaseUrl` to load an externally hosted build.
