...
export type WorkspaceLabels = {
  ...
  ownerLeaveWorkspaceQuestion: string;
  ownerLeaveWorkspaceDescription: string;
  exitWorkspace: string;
  exitWorkspaceDescription: string;
  temporarilyLeaveWorkspace: string;
  temporarilyLeaveWorkspaceDescription: string;
  workspaceSuspended: string;
  resumeWorkspace: string;
  rememberWorkspaceLeaveChoice: string;
  workspaceSettingsStyle: string;
  workspaceSettingsBehavior: string;
  workspaceSettingsMessaging: string;
  workspaceLeavePreference: string;
  workspaceLeavePreferenceAsk: string;
  workspaceLeavePreferenceSuspend: string;
  workspaceLeavePreferenceExit: string;
  ...
  saveBeforeDownload: string;
  saveBeforeCollaboration: string;
  returnToLibraryDescription: string;
  restoreOriginal: string;
  restoreOriginalQuestion: string;
  restoreOriginalDescription: string;
  confirmRestoreOriginal: string;
  ...
  localStatus: string; connectingStatus: string; connectedStatus: string; syncingStatus: string; availableStatus: string; ownerOfflineStatus: string; unavailableStatus: string; history: string; currentVersion: string; initialVersion: string; version: string; collaborationDeleteBlocked: string; collaborationOnlyOne: string; proposalStale: string; joinedPermanentLink: string; proposalPreview: string; parameterResult: string; currentStep: string; rollbackToStep: string; parameterActions: string; language: string; english: string; chinese: string; workspaceVersion: string; cursorPosition: string; cursorColor: string; moveCursorOverImage: string; moveCursorForColor: string; savePreviewChanges: string; savePreviewChangesDescription: string; discardPreviewChanges: string; discardPreviewChangesDescription: string; previewTransparencyTitle: string; previewTransparencyDescription: string; saveAsPng: string; panTool: string; selectTool: string; freeRotate: string; rotationDegrees: string; rotate: string; flip: string; undo: string; redo: string;
 };
...
const en: WorkspaceLabels = {
  ...
  workingProcessing: "Working · Processing", processCollaborate: "Process, doodle, and collaborate on selected images", workingEmpty: "Working is empty", addFromOrigin: "Add an image from Origin to begin processing.", workingViewMode: "Working view", workingListView: "List view", workingGridView: "Grid view", workingCompactView: "Compact view",
  ...
  stylePreview: "Style preview", imageInformation: "Image information", expandPanel: "Expand panel", collapsePanel: "Collapse panel", selectImage: "Select an image to inspect it.", workspaceOverview: "Workspace overview",
  ...
  leaveWorkspaceDescription: "Leaving disconnects the current collaboration session and returns to the home page.", ownerLeaveWorkspaceQuestion: "Leave workspace?", ownerLeaveWorkspaceDescription: "Choose an option.", exitWorkspace: "Exit", exitWorkspaceDescription: "Close the page. Data stays.", temporarilyLeaveWorkspace: "Suspend", temporarilyLeaveWorkspaceDescription: "Resume later.", workspaceSuspended: "Workspace suspended", resumeWorkspace: "Resume workspace", rememberWorkspaceLeaveChoice: "Remember", workspaceSettingsStyle: "Style", workspaceSettingsBehavior: "Behavior", workspaceSettingsMessaging: "Messaging", workspaceLeavePreference: "When leaving as Owner", workspaceLeavePreferenceAsk: "Ask every time", workspaceLeavePreferenceSuspend: "Suspend workspace", workspaceLeavePreferenceExit: "Exit workspace",
  stopCollaborationQuestion: "Stop collaboration?", stopCollaborationDescription: "Stop restores this Working card to its original cached thumbnail. Save & stop lets you overwrite it with the rendered result or create a new image first.", stoppingCollaboration: "Stopping...", stopDirectly: "Stop", saveAndStopCollaboration: "Save & stop", saveImageQuestion: "Save current image?", saveImageConfirmationDescription: "Choose whether to overwrite this image or create a new rendered image in Working.", unsavedChanges: "Unsaved changes", saveBeforeDownload: "Save the current changes before downloading this image.", saveBeforeCollaboration: "Save the current changes before starting collaboration.", returnToLibraryDescription: "Return the original image to Library and discard Working parameters.", restoreOriginal: "Restore original", restoreOriginalQuestion: "Restore the original image?", restoreOriginalDescription: "This clears all unsaved edits and returns the image to its last saved content.", confirmRestoreOriginal: "Restore",
...
const zh: WorkspaceLabels = {
  ...
  workingEmpty: "处理区为空", addFromOrigin: "从原始图区域添加图片开始处理。", workingViewMode: "处理区视图", workingListView: "列表", workingGridView: "宫格", workingCompactView: "紧凑", collaboration: "协作", collaborators: "协作者", noCollaborators: "暂无在线协作者", viewingWorkspace: "正在查看", directConnection: "设备直连", relayedConnection: "网络转发", packetLoss: "丢包率",
  activity: "动态", messages: "消息", noMessages: "暂无消息", workspaceSettings: "工作区设置", workspaceStyleEditor: "工作区样式编辑器", stylePreview: "样式预览", imageInformation: "图片信息", expandPanel: "展开面板", collapsePanel: "收起面板", selectImage: "选择图片后查看详细信息。",
