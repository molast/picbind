...
import { WorkspaceShareDialog } from "../dialogs/workspace-share-dialog";
import { WorkspaceWeixinChatDialog } from "../dialogs/workspace-weixin-chat-dialog";
...
export default function WorkspacePage({ shareToken, initialWorkspace, userDisplayName, publicSiteUrl, desktop = false, onSuspend, onExit }: {
  ...
  const [previewSidebarCollapsed, setPreviewSidebarCollapsed] = React.useState(true);
  const [workspaceSidebarCollapsed, setWorkspaceSidebarCollapsed] = React.useState(true);
  ...
  const persistWorkspaceLog = React.useCallback(async (workspaceId: string, kind: string, imageId?: string, detail?: unknown, actorId = "local") => {
    const value: WorkspaceActivity = { eventId: createPrefixedId("log"), sequence: Date.now(), actorId, kind, imageId, detail, createdAt: Date.now(), scope: "workspaceLog" };
  ...
  const persistCollaborationActivity = React.useCallback(async (workspaceId: string, kind: string, imageId: string, detail: unknown, actorId = "local") => {
    const value: WorkspaceActivity = { eventId: createPrefixedId("activity"), sequence: Date.now(), actorId, kind, imageId, detail, createdAt: Date.now(), scope: "collaborationActivity" };
  ...
  const updateImage = React.useCallback(async (imageId: string, patch: Partial<WorkspaceImage>) => {
    ...
    const latestImage = latest.find((candidate) => candidate.imageId === imageId);
    if (!latestImage) return;
    const cached = {
      ...latestImage,
      ...patch,
      sourceCached: latestImage.sourceCached || Boolean(patch.source),
      previewCached: latestImage.previewCached || Boolean(patch.preview),
      updatedAt,
      ...(Object.prototype.hasOwnProperty.call(patch, "source") ? { source: undefined, sourceAddress: undefined } : {}),
      ...(Object.prototype.hasOwnProperty.call(patch, "preview") ? { preview: undefined } : {}),
    };
    const next = latest.map((candidate) => candidate.imageId === imageId ? cached : candidate);
    imagesRef.current = next;
    setImages(next);
  }, []);
  const updateImageDimensions = React.useCallback(
    (imageId: string, width: number, height: number) => updateImage(imageId, { width, height }),
    [updateImage],
  );
  ...
  React.useEffect(() => {
    const previewImageId = activityPreview?.activity.imageId;
    if (!previewImageId || previewImageId === selectedId) return;
    clearCollaborationPreviewSnapshot(previewImageId);
  ...
  const workspaceMessaging = useWorkspaceMessaging({
    ...
    canAddImages: workspace?.role === "owner",
  ...
  const sendWorkspaceSnapshot = React.useCallback(async (targetUserId?: string) => {
    ...
    realtimeRef.current?.send("stateSnapshot", {
      images: sharedImages.map(({ source: _source, preview: _preview, sourceAddress: _sourceAddress, sourceCached: _sourceCached, previewCached: _previewCached, ...image }) => image),
      commits: sharedCommits,
      style: workspace.style,
    }, { route, targetUserId, delivery: "reliable" });
    await Promise.all(sharedImages.map(async (image) => {
      try {
        ...
        if (container) {
          await publishPreview(image, container.workingBlob, targetUserId);
          return;
        }
        const source = await readWorkspaceImageSource(image);
        if (source) await publishPreview(image, source, targetUserId);
      } catch (error) {
  ...
  const handleRealtimeEvent = React.useCallback((value: WorkspaceEvent | Record<string, unknown>) => {
    ...
    else if (type === "message") setMessages((current)=>[...current,{id:String(value.eventId||createPrefixedId("message")),text:String(value.text||""),actor:String(value.senderName||"Guest")}]);
  ...
  const { sourceTransfers, receiveSource, acceptSourceRequest, rejectSourceRequest } = useWorkspaceSourceTransfer({
    ...
    setSourceRequestDialog, setSourceRejectReason, setNotice, finishSourceRequest, syncCollaborationPreview, releaseCollaborationContainer,
  ...
  const { moveImageToWorking, requestMoveImageToWorking, requestDeleteImage, confirmDeleteImage } = useWorkspaceImageCommands({
    ...
    setPendingWorkingImageId, setMovingToWorkingImageIds, setCompressionSuggestionWeakNetwork,
    ...
    setNotice, updateImage, persistWorkspaceLog, releaseProcessingSource, releaseCollaborationContainer,
    collaborationDeleteBlockedMessage: workspaceText("collaborationDeleteBlocked"),
    imageActionFailedMessage: workspaceText("imageActionFailed"),
  });
  ...
  const { publishPreview, publishImage: togglePublishedImage } = useWorkspacePublishing({
    workspace, imagesRef, updateImage, syncCollaborationPreview,
  ...
  const { saveCollaborativeImage, saveCurrentImage } = useWorkspaceSaveCollaboration({
    workspace, selected, imagesRef, setCollaborationSaving, setNotice, updateImage,
    syncCollaborationContainer, syncCollaborationPreview, releaseCollaborationContainer,
    clearCollaborationHistory, saveProcessedCopy, persistCollaborationActivity,
    broadcastWorkspaceSnapshot: () => sendWorkspaceSnapshot(),
  });
  const restoreCurrentImage = React.useCallback(async (image: WorkspaceImage) => {
    const current = imagesRef.current.find((candidate) => candidate.imageId === image.imageId) || image;
    if (current.shared || !hasPendingWorkspaceImageChanges(current)) return;
    try {
      await updateImage(current.imageId, {
        currentCommitId: initialWorkspaceCommitId(current.imageId),
        parameterDocument: emptyImageParameterDocument(),
        state: "working",
      });
      releaseCollaborationContainer(current.imageId);
      if (workspace) {
        await persistWorkspaceLog(workspace.workspaceId, "imageChangesRestored", current.imageId);
      }
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to restore the original image");
    }
  }, [persistWorkspaceLog, releaseCollaborationContainer, setNotice, updateImage, workspace]);
  const requestImageDownload = React.useCallback(async (image: WorkspaceImage) => {
    const current = imagesRef.current.find((candidate) => candidate.imageId === image.imageId) || image;
    if (hasPendingWorkspaceImageChanges(current)) {
      setSelectedId(current.imageId);
      setPendingImageSaveAction({ imageId: current.imageId, action: "download" });
      return false;
    }
    return downloadImage(current);
  }, [downloadImage, setSelectedId]);
  const publishImage = React.useCallback(async (image: WorkspaceImage) => {
    const current = imagesRef.current.find((candidate) => candidate.imageId === image.imageId) || image;
    if (current.shared) {
      setStopCollaborationImage(current);
      return;
    }
    if (hasPendingWorkspaceImageChanges(current)) {
      setPendingImageSaveAction({ imageId: current.imageId, action: "collaborate" });
      return;
    }
    await togglePublishedImage(current);
  }, [setStopCollaborationImage, togglePublishedImage]);
  const completePendingImageSave = React.useCallback(async (choice: "copy" | "replace") => {
    const pending = pendingImageSaveAction;
    if (!pending || collaborationSaving) return;
    const image = imagesRef.current.find((candidate) => candidate.imageId === pending.imageId);
    if (!image) {
      setPendingImageSaveAction(null);
      return;
    }
    const savedImageId = await saveCurrentImage(choice, image);
    if (!savedImageId) return;
    const savedImage = imagesRef.current.find((candidate) => candidate.imageId === savedImageId);
    setPendingImageSaveAction(null);
    if (!savedImage) {
      setNotice("Saved image is unavailable");
      return;
    }
    try {
      if (pending.action === "download") {
        if (await downloadImage(savedImage)) showToast(workspaceText("downloadComplete"));
      } else {
        await togglePublishedImage(savedImage);
      }
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to continue after saving image");
    }
  }, [collaborationSaving, downloadImage, pendingImageSaveAction, saveCurrentImage, setNotice, showToast, togglePublishedImage]);
  ...
  const saveAndStopCollaboration = React.useCallback(async (choice: "copy" | "replace") => {
    const image = stopCollaborationImage;
    if (!image || stoppingCollaboration) return;
    setStoppingCollaboration(true);
    try {
      const saved = await saveCollaborativeImage(choice, image, { continueCollaboration: false });
      if (!saved) return;
  ...
  }, [saveCollaborativeImage, setNotice, setStopCollaborationImage, setStoppingCollaboration, stopCollaborationImage, stoppingCollaboration, togglePublishedImage]);
  const { createShare, rotateShare } = useWorkspaceShareCommands({
    workspace, displayName: userDisplayName, setWorkspace, setImages, realtimeRef, realtimeService,
    subscribe: (client) => client.subscribe((value) => realtimeEventRef.current(value)),
    transition: () => transitionRuntime({ type: "transition", next: "connecting" }), setNotice,
  });
  ...
  async function previewProposal(proposal: WorkspaceProposal) {
    try {
      ...
      const rendered = await renderCollaborationPreviewSnapshot(image, parameterDocument, proposal.commit?.commitId);
      if (!rendered?.url) throw new Error("Proposal preview is unavailable");
      setProposalPreview({ proposalId: proposal.proposalId, imageId: proposal.imageId, original, resultUrl: rendered.url });
    } catch (error) {
  ...
  async function previewCollaborationActivity(activity: WorkspaceActivity) {
    ...
    if (!snapshot && proposalId) {
      ...
      if (proposal && activity.kind === "proposalApproved") {
        ...
        if (isValidImageParameterDocument(historicalDocument)) {
          const rendered = await renderCollaborationPreviewSnapshot(selected, historicalDocument, commitId || proposal.commit?.commitId);
          if (!rendered?.url) throw new Error("Activity preview is unavailable");
          setActivityPreview({activity,parameterDocument:historicalDocument,previewUrl:rendered.url,commitId:commitId||undefined});
          return;
        }
      }
      if (proposal) {
        ...
        const rendered = await renderCollaborationPreviewSnapshot(image, parameterDocument, proposal.commit?.commitId);
        if (!rendered?.url) throw new Error("Proposal preview is unavailable");
        setProposalPreview({ proposalId: proposal.proposalId, imageId: proposal.imageId, original: input.source, resultUrl: rendered.url });
        setActivityPreview({ activity, parameterDocument, previewUrl: rendered.url });
        return;
      }
      if (workspace?.role === "collaborator" && activity.actorId === "local") {
        ...
        const operation: WorkspaceOperation = {
          operationId: typeof detail?.operationId === "string" ? detail.operationId : createPrefixedId("activity-operation"),
          imageId: selected.imageId,
          authorId: "local",
          baseCommitId: typeof detail?.commitId === "string" ? detail.commitId : selected.currentCommitId || initialWorkspaceCommitId(selected.imageId),
          type: operationType,
    ...
    try {
      ...
      const rendered = await renderCollaborationPreviewSnapshot(selected, parameterDocument, commitId || undefined);
      if (!rendered?.url) throw new Error("Activity preview is unavailable");
      setActivityPreview({activity,parameterDocument,previewUrl:rendered.url,commitId:commitId||undefined});
    } catch (error) {
  ...
  function cancelActivityPreview() {
    if (activityPreview?.activity.imageId) clearCollaborationPreviewSnapshot(activityPreview.activity.imageId);
    if (proposalPreview?.imageId) clearCollaborationPreviewSnapshot(proposalPreview.imageId);
    setActivityPreview(null);
  ...
  return <main data-picbind-workspace-content="true" className="flex h-screen min-h-0 flex-col overflow-hidden bg-[#f3f5f8] text-[#172033]">
    {!maximizedWorkspaceImage ? <WorkspaceHeader workspace={workspace} runtime={runtime} onlinePeers={onlinePeers} collaborationOpen={collaborationOpen} desktop={desktop} lang={lang} onLanguageChange={(nextLang)=>{persistLang(nextLang);setLanguage(nextLang);}} onEnterWorkspace={()=>setShareIdEntryOpen(true)} onLeave={requestLeave} onToggleCollaboration={()=>setCollaborationOpen((value)=>!value)} onShare={()=>setShareDialogOpen(true)} onSettings={()=>setSettingsOpen(true)} /> : null}
    ...
    <div className={`grid min-h-0 flex-1 grid-cols-1 overflow-y-auto lg:overflow-hidden ${(maximizedWorkspaceImage ? previewSidebarCollapsed : workspaceSidebarCollapsed) ? "lg:grid-cols-[minmax(0,1fr)_36px]" : "lg:grid-cols-[minmax(0,1fr)_clamp(320px,24vw,420px)]"}`}>
      ...
      {maximizedWorkspaceImage ? <WorkspacePreviewSidebar image={maximizedWorkspaceImage} source={maximizedPreviewMemory?.surface} width={maximizedPreviewMemory?.width || maximizedWorkspaceImage.width} height={maximizedPreviewMemory?.height || maximizedWorkspaceImage.height} selection={previewSelection} cursor={previewCursor} collapsed={previewSidebarCollapsed} onToggle={() => setPreviewSidebarCollapsed((value) => !value)} /> : <WorkspaceSidebar collapsed={workspaceSidebarCollapsed} onToggle={() => setWorkspaceSidebarCollapsed((value) => !value)} label={workspaceText("workspaceOverview")} expandLabel={workspaceText("expandPanel")} collapseLabel={workspaceText("collapsePanel")}>
    ...
    <WorkspaceShareDialog open={shareDialogOpen} lang={lang} role={workspace.role} shareId={workspace.shareToken || ""} shareLink={workspace.shareToken && (publicSiteUrl || typeof window !== "undefined") ? shareUrl(workspace.shareToken, publicSiteUrl) : ""} onClose={()=>setShareDialogOpen(false)} onCreate={createShare} onCopySuccess={(target)=>showToast(workspaceText(target === "link" ? "shareLinkCopied" : "workspaceIdCopied"))} />
    <WorkspaceShareIdEntryDialog open={shareIdEntryOpen} lang={lang} desktop={desktop} onClose={() => setShareIdEntryOpen(false)} />
    <WorkspaceWeixinChatDialog open={workspaceMessaging.chatOpen} provider={workspaceMessaging.activeProvider} messages={workspaceMessaging.messages} labels={messagingLabels} sending={workspaceMessaging.sending} sendingImage={workspaceMessaging.sendingImage} canMoveImages={workspace.role === "owner"} onSend={workspaceMessaging.sendText} onOpenImagePicker={()=>setMessagingImagePickerOpen(true)} onMoveImage={workspaceMessaging.moveImageToLibrary} onLoadImage={workspaceMessaging.loadImage} onClose={()=>{setMessagingImagePickerOpen(false);workspaceMessaging.closeChat();}} />
