...
export function useWorkspaceShareCommands({ workspace, displayName, setWorkspace, setImages, realtimeRef, realtimeService, subscribe, transition, setNotice, }: {
  ...
  const createShare = React.useCallback(async () => {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : "Unable to create the share link");
    }
  }, [displayName, realtimeRef, realtimeService, setImages, setNotice, setWorkspace, subscribe, transition, workspace]);
  ...
  const rotateShare = React.useCallback(async () => {
    ...
    try {
      ...
      setNotice(error instanceof Error ? error.message : "Unable to create a new share link");
    }
