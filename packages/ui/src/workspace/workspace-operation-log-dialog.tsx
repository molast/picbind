  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-4" onMouseDown={(event)=>event.target===event.currentTarget&&onClose()}>
    <section className="flex h-[min(72vh,620px)] w-full max-w-4xl flex-col overflow-hidden rounded-lg border border-slate-700 bg-[#0b1020] shadow-2xl" role="dialog" aria-modal="true" aria-label={text("operationLogTitle")}>
      ...
      <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto px-4 py-3 font-mono text-[11px] leading-5 [scrollbar-color:#475569_transparent]">
        {logs.length?logs.map((log)=><div key={log.eventId} className="grid grid-cols-[150px_180px_minmax(0,1fr)] gap-2 border-b border-slate-800/70 py-1.5 last:border-b-0"><span className="text-slate-500">[{new Date(log.createdAt).toLocaleString()}]</span><span className="truncate uppercase text-cyan-300">{log.kind}</span><span className="min-w-0 whitespace-pre-wrap break-words text-slate-300">{log.imageId?<span className="text-slate-100">{log.imageId} </span>:null}{log.detail!==undefined?JSON.stringify(log.detail):""}</span></div>):<div className="flex h-full items-center justify-center text-slate-600">$ {text("noOperationLogs")}</div>}
      </div>
    </section>
  </div>;
}
