...
import { FiClock, FiCrop, FiDownload, FiEye, FiHardDrive, FiImage, FiLink, FiRefreshCw, FiShield, FiShare2, FiSliders, FiTrash2 } from "react-icons/fi";
...
import { WorkspaceSavePopover } from "../dialogs/workspace-save-dialog";
...
export function WorkspaceImageSidebar({ selected, selectedIsLibrary, shareId, role, runtime, imagesCount, workingCount, collaborators, commits, activities, proposals, selectedOriginalCommit, requestingSource, previewBlob, collaborationSaving, onPublish, onDelete, onRequestSource, onOperation, onSave, onActivity, onOriginal, onRollback, onCreateShare, onRotateShare, onCopySuccess, hasShareToken }: { selected?: WorkspaceImage | null; selectedIsLibrary: boolean; shareId: string | null; role: WorkspaceIdentity["role"]; runtime: string; imagesCount: number; workingCount: number; collaborators: Collaborator[]; commits: WorkspaceCommit[]; activities: WorkspaceActivity[]; proposals: WorkspaceProposal[]; selectedOriginalCommit?: WorkspaceCommit; requestingSource: boolean; previewBlob?: Blob; collaborationSaving: boolean; onPublish(image: WorkspaceImage): void; onDelete(image: WorkspaceImage): void; onRequestSource(image: WorkspaceImage): void; onOperation(image: WorkspaceImage, operation: "crop" | "adjust" | "review"): void; onRollback(commit: WorkspaceCommit): void; onCreateShare(): void; onRotateShare(): void; onCopySuccess(): void; onSave(choice: CollaborationSaveChoice): Promise<boolean>; onActivity(activity: WorkspaceActivity): void; onOriginal(): void; hasShareToken: boolean }) {
  const workspaceId = shareId || "";
  const [workspaceIdCopied, setWorkspaceIdCopied] = React.useState(false);
  const [saveOpen, setSaveOpen] = React.useState(false);
  React.useEffect(() => setSaveOpen(false), [selected?.imageId]);
