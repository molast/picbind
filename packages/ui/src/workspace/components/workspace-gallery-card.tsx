...
import { FiBookmark, FiCheck, FiDownload, FiLoader, FiMaximize2, FiMoreHorizontal, FiTrash2 } from "react-icons/fi";
...
export function WorkspaceGalleryCard({
  ...
  return <article className={`relative min-w-0 rounded-md border bg-white transition ${selected ? "border-[#2f65cf] shadow-[0_0_0_2px_#2f65cf]" : "border-slate-200 hover:border-slate-300"}`}>
    ...
    <button type="button" onClick={onSelect} className="block w-full px-3 pt-3 text-left">
      <div className="flex min-w-0 items-center justify-between gap-2">
        <strong className="truncate text-sm font-semibold text-slate-800">{image.name}</strong>
        {image.shared ? <span className="inline-flex h-5 shrink-0 items-center rounded border border-emerald-200 bg-emerald-50 px-1.5 text-[10px] font-bold text-emerald-700">{text("collaborating")}</span> : null}
      </div>
    </button>
    <div className="flex min-h-12 items-center justify-between gap-2 px-3 pb-3 pt-1 text-xs text-slate-500">
      <span className="min-w-0">
        <span className="block">{hasParameters ? `${text("sourceFileSize")} · ` : ""}{bytes(image.size)}</span>
        <span className="block text-[10px] text-slate-400">{renderedDimensions.width} × {renderedDimensions.height}</span>
      </span>
      <span className="flex shrink-0 items-center gap-1.5">
        {role !== "owner" && !hasSource ? <button type="button" onClick={onRequestSource} disabled={!onlinePeers || requestingSource} className="flex h-7 w-7 items-center justify-center rounded-md bg-[#2f65cf] text-white disabled:opacity-60" title={requestingSource ? text("requestingSource") : text("requestSource")} aria-label={requestingSource ? text("requestingSource") : text("requestSource")}>
          {requestingSource ? <FiLoader className="h-3.5 w-3.5 animate-spin" /> : <FiBookmark className="h-3.5 w-3.5" />}
        </button> : null}
        {hasSource ? <button type="button" onClick={() => void handleDownload()} disabled={downloadState === "downloading"} className={`flex h-7 w-7 items-center justify-center rounded-md border transition ${downloadState === "downloaded" ? "border-emerald-200 bg-emerald-50 text-emerald-600" : downloadState === "downloading" ? "border-blue-200 bg-blue-50 text-[#2f65cf]" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`} title={downloadState === "downloaded" ? text("downloadComplete") : downloadState === "downloading" ? text("downloading") : text("download")} aria-label={downloadState === "downloaded" ? text("downloadComplete") : downloadState === "downloading" ? text("downloading") : text("download")} aria-live="polite">{downloadState === "downloaded" ? <FiCheck className="h-3.5 w-3.5" /> : downloadState === "downloading" ? <FiLoader className="h-3.5 w-3.5 animate-spin" /> : <FiDownload className="h-3.5 w-3.5" />}</button> : <button type="button" disabled className="flex h-7 w-7 items-center justify-center rounded-md border border-slate-200 text-slate-300" title={text("sourceUnavailable")}><FiDownload className="h-3.5 w-3.5" /></button>}
      </span>
    </div>
  </article>;
}
