...
v2.0 在共享 `packages/wasm/image-wasm` 中补充以下能力：
...
修改 Rust 或 WASM API 后必须重新生成 `packages/wasm/image-wasm` 产物；如果改动同时影响压缩行为或公开压缩配置，还必须同步更新仓库根目录的 `COMPRESSION_ALGORITHM.md`。
