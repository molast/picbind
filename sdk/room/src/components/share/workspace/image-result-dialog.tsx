...
const STAGE_LABELS: Record<ProcessedImageActionStage, string> = {
  preparing: "正在准备图片",
  waiting: "预览信息已发送，等待对方决定是否接收",
  transferring: "对方已接受，正在发送图片",
  complete: "处理完成",
};

export default function ImageResultDialog({ source, result, onClose, onAction, onResolveRejected }: ImageResultDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[96] flex items-center justify-center bg-slate-950/45 p-4">
      <section className="w-full max-w-md overflow-hidden rounded-lg bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="图片处理结果">
        <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div><h2 className="text-base font-semibold text-slate-900">图片处理完成</h2><p className="mt-0.5 max-w-xs truncate text-xs text-slate-500">{result.name}</p></div>
          <button type="button" disabled={running || Boolean(rejectedImageId)} onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 disabled:opacity-30" aria-label="关闭"><FiX className="h-4 w-4" aria-hidden="true" /></button>
        </header>
        <div className="space-y-4 p-5">
          <div className="aspect-video overflow-hidden rounded-md bg-slate-100"><img src={previewUrl} alt="处理结果预览" className="h-full w-full object-contain" /></div>
          ...
          {stage || error || rejectedImageId ? (
            <div className={`flex items-start gap-3 rounded-md px-3 py-3 text-xs ${error ? "bg-red-50 text-red-700" : rejectedImageId ? "bg-amber-50 text-amber-800" : stage === "complete" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-[#2f65cf]"}`}>
              ...
              <span>{error || (rejectedImageId ? "对方拒绝接收这张图片，是否保存到左侧图片列表？" : action === "store" && stage === "complete" ? "已存入左侧本地图片列表" : stage ? STAGE_LABELS[stage] : "")}</span>
            </div>
          ) : null}
        </div>
        <footer className="flex justify-end gap-2 border-t border-slate-200 px-5 py-4">
          {rejectedImageId ? (
            ...
            <button type="button" onClick={onClose} className="h-9 rounded-md bg-[#2f65cf] px-4 text-xs font-semibold text-white hover:bg-[#2457bd]">{error ? "关闭" : "完成"}</button>
          ) : (
            <>
              <button type="button" disabled={running} onClick={() => void start("store")} className="inline-flex h-9 items-center gap-1.5 rounded-md border border-slate-200 px-4 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-40">
                ...
                {running && action === "store" ? "存储中" : "存储到本地"}
              </button>
              <button type="button" disabled={running} onClick={() => void start("share")} className="inline-flex h-9 items-center gap-1.5 rounded-md bg-[#2f65cf] px-4 text-xs font-semibold text-white hover:bg-[#2457bd] disabled:opacity-40">
                ...
                {running && action === "share" ? (stage === "waiting" ? "等待接收" : stage === "transferring" ? "发送中" : "准备中") : "分享给对方"}
              </button>
            </>
          )}
        </footer>
