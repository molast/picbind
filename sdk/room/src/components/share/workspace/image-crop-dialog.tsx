...
const RATIOS: Array<{ label: string; value: RatioValue; ratio: number | null }> = [
  ...
  { label: "原始", value: "original", ratio: -1 },
...
export default function ImageCropDialog({ image, onClose, onSave }: ImageCropDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/45 p-4">
      <section className="flex max-h-[94vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="裁剪图片">
        <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <h2 className="text-base font-semibold text-slate-900">裁剪图片</h2>
            <p className="mt-0.5 max-w-md truncate text-xs text-slate-500">{image.name}</p>
          </div>
          <button type="button" onClick={onClose} disabled={working} className="flex h-8 w-8 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100 disabled:opacity-40" aria-label="关闭"><FiX className="h-4 w-4" aria-hidden="true" /></button>
        </header>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <div className="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-500">
            <span>输出尺寸：{cropWidth} × {cropHeight}</span>
