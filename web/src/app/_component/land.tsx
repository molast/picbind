  ...
  return (
    <div id="photoshow-land" className="max-w-screen-sm mx-auto">
      ...
      <section className="upload w-full mt-8">
        ...
        {/* AI暂时先隐藏 */}
        {/* <div className="w-full text-sm text-center text-slate-500 mt-4">
          {Locale.Photo.Landing.Or}
        </div>
        <div className="flex w-full items-center mt-4 flex-wrap space-y-2 md:flex-nowrap md:space-y-0 md:space-x-2">
          <Input
            onChange={handlePromptChange}
            className=" bg-white text-slate-600 text-sm"
            type="email"
            placeholder={Locale.Photo.Landing.CreateImagePlaceholder}
          />
          <div className="flex items-center md:w-full justify-between md:space-x-2 flex-1">
            ...
            {["aura-flow", "qr-code"].includes(model.value) ? (
              <Button
                ...
                disabled={
                  (model.value === "aura-flow" && !prompt) ||
                  (model.value === "qr-code" && !link)
                }
                size={"sm"}
                type="submit"
                className={"px-6 text-sm"}
              >
                {Locale.Photo.Landing.CreateImageAction}
              </Button>
            ) : (
              <RatioModal
                ratio={ratio}
                setRatio={setRatio}
                confirm={() => handleOngenerateImage()}
                disabled={!prompt}
              />
            )}
          </div>
        ...
        {prompt && message && (
          <div className="w-full p-2 text-sm text-red-500 text-center">
            {message}
          </div>
        )}
        {model.value === "qr-code" && (
          <div className="w-full mt-4 bg-white">
            <Input
              ...
              className=" bg-white text-slate-600 text-sm"
              placeholder={Locale.Photo.Landing.InputLinkPlaceholder}
            />
          </div>
        )} */}
      </section>
      ...
      <section className="example mt-8">
        <div className="w-full text-sm text-center text-slate-500">
          {Locale.Photo.Landing.NowSupport}
        </div>
