  ...
  return (
    <div className="w-full flex justify-center">
      {!payload.mask ? (
        ...
        <div className="w-full p-2 bg-violet-200 rounded-full flex justify-between items-center overflow-hidden">
          <div className="flex-1 px-1 flex items-center space-x-2">
            ...
            <p className="flex-1 text-slate-500 text-sm overflow-hidden">
              {payload.mask.name}
            </p>
          </div>
          <div
      ...
      <input
        ...
        onChange={(ev) => handleFileSelect(ev.currentTarget.files ?? [])}
      />
    </div>
  );
}
