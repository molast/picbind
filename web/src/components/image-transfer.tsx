...
function ImageTransfer({
    ...
    try {
      ...
      if (payload.canvas) {
        if (payload.model === "runway") {
          const newCanvas = (await ImageManager.resetSizeCanvas(
            payload.canvas,
  ...
  const handleStart = async () => {
    try {
      ...
      // 因为图片尺寸有变化
      if (
        ["crop-img", "uncrop", "filter-img", "character"].includes(tool.name)
      ) {
        setSrc(local);
      }
      // 设置结果
  ...
  const handleContinue = async () => {
    ...
    if (tool.name === "character") {
      setTimeout(() => {
        setPayload((preData: any) => {
          return { ...preData, ratio: 1 / 1, label: "1:1" };
        });
      }, 20);
    }
    updTask({});
    ...
    <div id="image-transfer" className="w-full h-full space-y-4 flex flex-col">
      ...
      <div className="show w-full grow flex flex-col justify-center items-center space-y-4">
        ...
        <div className="relative w-full flex-1 flex items-center justify-center">
          ...
          <ZoomBox
            move={
              !["crop-img", "filter-img", "character", "stitching"].includes(
                tool.name,
              )
            }
            tool={tool}
          ...
          >
            ...
            <div
              className={twMerge(
                "flex-1 w-full space-y-2 flex items-center justify-center",
                ["inpaint-img", "remove-obj", "uncrop"].includes(tool.name)
                  ? "pb-12 h-full"
                  : "",
              )}
              style={{
                width: "200px",
                maxWidth:
                  ["remove-obj", "inpaint-img", "uncrop"].includes(tool.name) &&
                  !result
                    ? "100%"
                    : maxWidth,
              }}
              ...
              {![
                ...
                >
                  ...
                  {status === "Pending" && (
                    <div
                      className={twMerge(
                        "scan w-full absolute top-0 transition-all duration-200 pointer-events-none",
                      )}
                    ></div>
                  )}
                </div>
                ...
                >
                  ...
                  {videoSrc && media === "video" && (
                    <div
                      ...
                      style={{ background: "rgb(245, 245, 245, 0.6)" }}
                    >
                      <VideoPlayer url={videoSrc} width="100%" height="100%" />
                    </div>
                  )}
        ...
        <div className="w-full justify-center items-center">
          {status === "Pending" && (
            <div className="text-center text-sm text-violet-500">
              {media === "image" && <p>{Locale.System.WaitImage}</p>}
              {media === "video" && <p>{Locale.System.WaitVideo}</p>}
              {media === "text" && <p>{Locale.System.WaitText}</p>}
            </div>
          )}
              ...
              {tool.name === "translate-text" && !result && (
                <div className="w-full flex flex-col space-y-4 items-center justify-center">
                  <div className="w-full flex space-x-4 justify-center">
                    <LangSelecter
                      title={Locale.Photo.LangSelecter.Title}
                      langs={Locale.Photo.LangSelecter.List}
                      lang={payload.srcLang}
                      setLang={(lang) =>
                        setPayload((preData: any) => {
                          return { ...preData, srcLang: lang };
                        })
                      }
                    />
                    ...
                    <LangSelecter
                      title={Locale.Photo.LangSelecter.Title}
                      langs={Locale.Photo.LangSelecter.List.slice(1)}
                      lang={payload.tgtLang}
                      setLang={(lang) =>
                        setPayload((preData: any) => {
                          return { ...preData, tgtLang: lang };
                        })
                      }
                    />
                  </div>
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) => {
                        setPayload((preData: any) => {
                          return { ...preData, protectLang: value };
                        });
                      }}
                    />
                    <label
        ...
        <div className="flex-1 flex items-center justify-center space-x-2">
          {tool.name === "remove-obj" && result && (
            <Button variant="default" onClick={handleContinue}>
              {Locale.System.ContinueRemove}
            </Button>
          )}
          {tool.name === "uncrop" && result && (
            <Button variant="default" onClick={handleContinue}>
              {Locale.System.ContinueExpand}
            </Button>
          )}
          {tool.name === "inpaint-img" && result && (
            <Button variant="default" onClick={handleContinue}>
              {Locale.System.ContinueModify}
            </Button>
          )}
        </div>
        ...
        {!["read-text", "create-video", "character"].includes(tool.name) && (
          <div className="">
            {result || status === "Error" ? (
              ...
              <Button
                variant="default"
                disabled={status !== "Ready" || !isReady || media === "video"}
                onClick={handleStart}
              >
                {["crop-img", "filter-img"].includes(tool.name)
                  ? Locale.System.Save
                  : Locale.System.Start}
              </Button>
            )}
        ...
        {["character"].includes(tool.name) && (
          <div className="">
            {result || status === "Error" ? (
              ...
              <CharacterModal
                title={Locale.System.SelectFilter}
                status={status}
                payload={payload}
                setPayload={setPayload}
                confirm={handleStart}
