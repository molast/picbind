...
const en: LocaleType = {
  ...
  Photo: {
    ...
    Compress: {
      ...
      savedPercent: 'Compression ratio',
      compressing: 'Compressing...',
      waiting: 'Waiting',
      waitingResult: 'Waiting for compressed result',
      working: 'Compressing the image with WASM...',
      doneHint: 'Single-image compression is complete. Download from the top area, or continue with the batch list.',
      emptyHint: 'Upload an image first, or use the batch area below to compress multiple images.',
      dropzoneTitle: 'Drag multiple images here, or click to select',
      dropzoneLimit: (max: number) => `Up to ${max} images, 5MB max each`,
      selectedCount: (count: number) => `Selected ${count} image(s)`,
      batchHint: 'Batch compression supports comparison in the top area and per-image download',
      savedShort: (percent: number) => `Saved ${percent}%`,
      compareAction: 'Compare',
      downloadAction: 'Download',
      failed: 'Compression failed',
      emptyList: 'No images in the batch list yet. Upload from the area above.',
      batchFooter: 'Batch compression uses the same WASM compressor as the main preview.',
      batchAction: 'Batch compress',
    },
    Tool: {
